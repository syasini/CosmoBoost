...
def _K_d_lift(K,d,s_sign=+1):
    
    if d == 1:
        return K._mlpl_d1
    
    elif (d>1): 
        K_d_minusone = _K_d_lift(K,d-1)
        ...
        return K.gamma*K_d_minusone + K.gamma*K.beta*(  C_l_plusone* K_l_plusone_d_minusone\
                    + s_sign*K.Smatrix*K_d_minusone + K.Cmatrix*K_l_minusone_d_minusone  )
...
def K_d(K,d):
    ...
    if d>=1:
        K_d_mlpl = _K_d_lift(K,d)
        return K_d_mlpl
        
    elif d<1:
        K_d_mlpl = _K_d_lift(K,2-d,s_sign=-1)
        print d
